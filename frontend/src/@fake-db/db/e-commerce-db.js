...
mock.onPost('/api/e-commerce-app/remove-orders').reply(request => {
	...
	objIndex = eCommerceDB.customers.findIndex((obj => obj.orderIds == orderIds));
	const index = eCommerceDB.customers[objIndex].orderIds.indexOf(orderIds);
	if (index > -1) {
		eCommerceDB.customers[objIndex].orderIds.splice(index, 1);
	  }
	  console.log(eCommerceDB.customers[objIndex].orderIds)
	// eCommerceDB.customers = eCommerceDB.customers.filter(customer => !orderIds.includes(order.id));
	return [200, orderIds];
});
...
mock.onGet('/api/e-commerce-app/customers').reply(() => {
	return [200, eCommerceDB.customers];
});

mock.onPost('/api/e-commerce-app/remove-customers').reply(request => {
	const { customerIds } = JSON.parse(request.data);
	eCommerceDB.customers = eCommerceDB.customers.filter(customer => !customerIds.includes(customer.id));
	return [200, customerIds];
});

mock.onGet('/api/e-commerce-app/customer').reply(request => {
	const { customerId } = request.params;
	const response = _.find(eCommerceDB.customers, { id: customerId });
	return [200, response];
});
